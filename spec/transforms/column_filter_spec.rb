...
describe ColumnFilter do
  describe "form properties" do
    it ".display_name" do
      ColumnFilter.display_name.should == "Column Filter"
    end
    it ".form_keys" do
      ColumnFilter.form_keys.should == [:columns]
    end
  end
  describe "#result" do
    it "should only show the specified columns" do
      table = Ruport::Data::Table.new(:data => [[1,2,3,4], [7,8,9,10]], :column_names => %w[a b c d])
      transform = ColumnFilter.new(table, { :columns => [ "a", "c"]})
